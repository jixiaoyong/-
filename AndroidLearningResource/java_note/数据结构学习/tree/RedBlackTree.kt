 ...
 *              5. 父节点 红色；叔节点 黑色 ； 为父节点的右节点 -> ❌ 父节点 涂黑 ；祖父节点 涂红，以祖父节点为支点，左旋，涂黑根节点
 *              6. 父节点 红色；叔节点 黑色 ； 为父节点的左节点 -> ❌ 父节点为支点，右旋
 *
 *              【注意】旋转
...
class RedBlackTree {
    ...
    private fun revolveLeft(node: RedBlackNode) {
        ...
        if (node.id < nodeSParent!!.id) {
            nodeSParent!!.left = right
        } else {
            nodeSParent!!.right = right
        }
    ...
    private fun revolveRight(node: RedBlackNode) {
        ...
        if (node.id < nodeSparent!!.id) {
            nodeSparent.left = left
        } else {
            nodeSparent.right = left
        }
...
fun main(args: Array<String>) {
    ...
    rbTree.insert(RedBlackNode(837))
